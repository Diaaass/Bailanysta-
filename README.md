...
| `npm test` | прогнать юнит- и интеграционные тесты |
...
**Юнит** — чистые функции без базы: схемы валидации, форматирование времени, генерация аватаров, счётчик лимита запросов.
...
- **Нет e2e-тестов.** Слой запросов покрыт интеграционными тестами, но пользовательские сценарии целиком (регистрация → пост → лайк → поиск) проверяются вручную. Playwright в проекте уже есть — он используется для скриншотов, так что сценарии добавляются поверх него.
...
17 коммитов, около 5271 строк, 7 страниц, 12 API-эндпоинтов, 19 компонентов, 63 теста.
