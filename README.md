...
**Сквозные** — семнадцать сценариев в настоящем браузере поверх production-сборки: регистрация, выход и повторный вход, публикация, редактирование, лайк с проверкой после перезагрузки, комментарий, подписка и переключение ленты, правка профиля, поиск, переключение темы, защита маршрутов, работа с клавиатуры: ссылка «к содержимому», ловушка фокуса в диалоге и отправка поста по Ctrl+Enter. Каждый спек заводит собственных пользователей со случайными именами, поэтому прогоны не мешают друг другу.
...
23 коммита, около 7419 строк, 10 страниц, 13 API-эндпоинтов, 26 компонентов, 101 тест плюс 17 сквозных сценариев.
